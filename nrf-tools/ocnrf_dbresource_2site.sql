...
/*
	1. This MySQL script is intended to be run before the deployment of a 2-site Geo-Redundant NRF and the database replication MUST be UP between both the sites.
	2. This MySQL script needs to be run ONLY on ONE of the MySQL nodes of ONLY one site(among the the 2 sites present).
	...
	5. CAUTION: Check if OCNRF-Privileged-User already exists by running the following query in the mysql prompt(logged in as root MySQL User):
		...
		If the result is NOT an empty set, please comment out the line(number 35) which is creating the OCNRF Privileged-User in the below script.
	6. CAUTION: Check if OCNRF-Application-User already exists by running the following query in the mysql prompt(logged in as root MySQL User):
		...
		If the result is NOT an empty set, please comment out the line(number 36) which is creating the OCNRF APPLICATION User in the below script.
	7. Copy this MySQL script to one of the MySQL nodes of the site where it will be run.
		For example:
			$ kubectl cp ocnrf_dbresource_2site.sql ndbmysqld-0:/home/mysql -n chicago -c mysqlndbcluster
	...
	9. Assuming that this MySQL script is in the present working directory, it needs to be run(as root MySQL User) as shown below:
			...
			total 4
			-rw-------. 1 mysql mysql 1695 Jun 10 04:12 ocnrf_dbresource_2site.sql
			[mysql@ndbmysqld-0 ~]$
			[mysql@ndbmysqld-0 ~]$ mysql -h 127.0.0.1 -uroot -p < ocnrf_dbresource_2site.sql
			Enter password:
			[mysql@ndbmysqld-0 ~]$
*/
