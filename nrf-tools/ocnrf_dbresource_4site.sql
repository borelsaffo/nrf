...
/*
	1. This MySQL script is intended to be run before the deployment of a 4-site Geo-Redundant NRF and the database replication MUST be UP between all four sites.
	2. This MySQL script needs to be run ONLY on ONE of the MySQL nodes of ONLY one site(among the the 4 sites present).
	...
	5. CAUTION: Check if OCNRF-Privileged-User already exists by running the following query in the mysql prompt(logged in as root MySQL User):
		...
		If the result is NOT an empty set, please comment out the line(number 42) which is creating the OCNRF Privileged-User in the below script.
	6. CAUTION: Check if OCNRF-Application-User already exists by running the following query in the mysql prompt(logged in as root MySQL User):
		...
		If the result is NOT an empty set, please comment out the line(number 43) which is creating the OCNRF APPLICATION User in the below script.
	7. Copy this MySQL script to one of the MySQL nodes of the site where it will be run.
		For example:
			$ kubectl cp ocnrf_dbresource_4site.sql ndbmysqld-0:/home/mysql -n <namespace> -c <containerName, like: mysqlndbcluster>
	...
	9. Assuming that this MySQL script is in the present working directory, it needs to be run(as root MySQL User) as shown below:
			...
			total 4
			-rw-r--r--. 1 mysql mysql 3861 Nov 11 08:27 ocnrf_dbresource_4site.sql
			[mysql@ndbmysqld-0 ~]$
			[mysql@ndbmysqld-0 ~]$ mysql -h 127.0.0.1 -uroot -p < ocnrf_dbresource_4site.sql
			Enter password:
			[mysql@ndbmysqld-0 ~]$
...
CREATE DATABASE IF NOT EXISTS commonConfigurationDB4 CHARACTER SET utf8;
...
CREATE DATABASE IF NOT EXISTS leaderElectionDB4 CHARACTER SET utf8;
...
GRANT SELECT, INSERT, CREATE, ALTER, DROP, LOCK TABLES, CREATE TEMPORARY TABLES, DELETE, UPDATE, EXECUTE ON commonConfigurationDB4.* TO 'nrfPrivilegedUsr'@'%';
...
GRANT SELECT, INSERT, CREATE, ALTER, DROP, LOCK TABLES, CREATE TEMPORARY TABLES, DELETE, UPDATE, EXECUTE ON leaderElectionDB4.* TO 'nrfPrivilegedUsr'@'%';
